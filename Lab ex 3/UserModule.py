...
def encode_dict_to_qr(data_dict, filename='qr_code.png', scale=0.25):
...
def add_users_manually(user_list):
    dict1 = get_user_input()
    add_user_to_list(dict1, user_list)


def smartScan(filename, user_list):
    data_dict = decode_qr_code(filename)
...
def validation(data_dict):
    ...
    if pattern.search(data_dict["name"]):
        return 1
    elif pattern.search(data_dict["gender"]):
        return 2
    elif not pattern2.match(data_dict["phone"]):
        return 3
    elif pattern3.search(data_dict["username"]):
        return 4
    elif not pattern4.match(data_dict["email"]):
        return 5
    else:
        return 0
